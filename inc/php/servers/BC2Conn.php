...
class BC2Conn {
	...
	private $_globalVars = array(
		"mapsFileXML" => "inc/php/servers/mapNames.xml",
		"playmodesFileXML" => "inc/php/servers/playModes.xml",
		"squadnamesFileXML" => "inc/php/servers/squadNames.xml",
		"teamnamesFileXML" => "inc/php/servers/teamNames.xml",
	 ...
	 * @param Integer - Server RCON query port
         * @param Float (optional) - timeout in seconds
	 * @param String (optional) - for debugging, use "-d"
	 */
	function __construct($serverIP, $serverRconQueryPort, $timeout = 5, $debug = null) {
	...
	private function _openConnection($debug = null) {
		...
		//	stream_set_timeout($this->_sock, $this->_timeout);
			...
			if($debug == "-d") {
				@$this->_sock = fsockopen("tcp://" . $this->_serverIP, $this->_serverRconQueryPort, $errno, $errstr, $this->_timeout);
				if(!$this->_sock) {
